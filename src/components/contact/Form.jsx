...
// NOTE: Uploaded file path (for tooling): /mnt/data/Screenshot 2025-11-21 114800.png
// (Per request, this code does NOT include any hidden inputs or attach this file.)
...
const FORMSPREE_ENDPOINT = "https://formspree.io/f/movbayzk";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage(
          data?.error ||
            "Failed to send form. Please try again or contact me directly."
        );
      }
    } catch (err) {
      console.error(err);
      setStatus("error");
      setMessage("Network error while sending form. Please try again.");
    }
  };
    ...
    <div>
      ...
      <div className="mx-2">
        <form
          ...
          // Formspree accepts form fields in multipart/form-data
        >
          <input
            name="name"
            type="text"
          ...
          <textarea
            name="message"
            placeholder="Message*"
            className={`${commonClass} h-22 resize-none`}
            required
          />
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {status === "sending" ? "Sending..." : "Submit"} {telegramSVG}
            </button>
